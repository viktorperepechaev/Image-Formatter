...
DarkenOperation::DarkenOperation(const std::uint8_t percent) : needed_share_(0.01 * percent) {}

DarkenOperation::DarkenOperation(const std::vector<std::string>& arguments) : DarkenOperation(std::stoi(arguments[0])) {}
...
bool DarkenOperation::ValidateArguments(const std::vector<std::string>& arguments) {
  return arguments.size() == 1
    && arguments[0].find_first_not_of("0123456789") == std::string::npos
    && 0 <= std::stoi(arguments[0])
    && std::stoi(arguments[0]) <= 100;
}
