...
RotateOperation::RotateOperation(const int rotation_degree) : rotation_degree_(rotation_degree) {}

RotateOperation::RotateOperation(const std::vector<std::string>& arguments) : rotation_degree_(std::stoi(arguments[0])) {}
...
void RotateOperation::Apply(Image& image) const {
  ...
  if (is_positive) {
    for (int counter = 0; counter < normalized_rotation_degree / 90; ++counter) {
...
bool RotateOperation::ValidateArguments(const std::vector<std::string>& arguments) {
